/**
 * @file main.cc
 * @brief SV Subscriber (Immediate Mode) — standalone terminal application
 *
 * Usage:
 *   ./sv_subscriber_imm --interface eth0 [--filter "ether proto 0x88ba"]
 *
 * Captures IEC 61850 Sampled Values from the network using pcap immediate
 * mode + pcap_next_ex, decodes them, and prints one line per sample to stdout.
...
int main(int argc, char **argv)
{
    ...
    std::fprintf(stderr, "Capturing SV on %s [immediate mode] (Ctrl+C to stop)...\n", iface);
